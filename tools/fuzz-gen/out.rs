...
fn check_ptr_to_ref(ctx: *const *mut VmCtx) -> TestResult {
    init();
    let result = ptr_to_ref(ctx);
    TestResult::passed()
}
#[quickcheck_macros::quickcheck]
fn check_transmut_netlist(nl: *const Netlist) -> TestResult {
    init();
    let result = transmut_netlist(nl);
    TestResult::passed()
}
#[quickcheck_macros::quickcheck]
fn check_ffi_load_vec(ptr: *mut u8, len: usize) -> TestResult {
    init();
    let result = ffi_load_vec(ptr, len);
    TestResult::passed()
}
#[quickcheck_macros::quickcheck]
fn check_ffi_load_cstr(ptr: *const c_char) -> TestResult {
    init();
    let result = ffi_load_cstr(ptr);
    TestResult::passed()
}
#[quickcheck_macros::quickcheck]
fn check_ffi_load_cstr_as_vec(ptr: *mut u8) -> TestResult {
    init();
    let result = ffi_load_cstr_as_vec(ptr);
    TestResult::passed()
}
#[quickcheck_macros::quickcheck]

...
#[requires(trace_safe(trace, ctx.memlen) && ctx_safe(ctx))]
#[ensures(trace_safe(trace, ctx.memlen) && ctx_safe(ctx))]
#[ensures(two_effects!(old(trace), trace, Effect::FdAccess, Effect::WriteN(count)) && result <= cnt)]
...
pub fn trace_pread(ctx: &mut VmCtx, fd: HostFd, ptr: SboxPtr, cnt: usize) -> usize {
    let slice = ctx.slice_mem_mut(ptr, cnt as u32);
    let os_fd: usize = fd.into();
    os_pread(os_fd, slice, cnt)
}
...
pub fn trace_write(ctx: &mut VmCtx, fd: HostFd, ptr: SboxPtr, cnt: usize) -> usize {
    let slice = ctx.slice_mem_mut(ptr, cnt as u32);
    let os_fd: usize = fd.into();
    os_write(os_fd, slice, cnt)
}
